...
/* Create a parcel */
			...
			/*
				not needed for this call - needed maybe when going from collection to locker
				'target_machine_id' => 'UKLEE8372',
		...
		$postcode		= $row['o_postcode_del'];
			*/
			if (strlen($add_2)>0)	{$add_1 = $add_1.', '.$add_2;}
			if (strlen($add_3)>0)	{$add_1 = $add_1.', '.$add_3;}
$add_1 = ucwords(strtolower($add_1));
			$building_number = explode(" ", $add_1);
			$street 		= str_replace($building_number[0], "", $add_1);

//die($postcode);
$postcode = strtoupper($postcode);
	$add_4 = strtoupper($add_4);	
if (strlen($add_4)==0){
	$add_4 = strtoupper($add_3);	
}
if (strlen($add_4)==0){
	$add_4 = strtoupper($add_2);	
}

$mobile = ltrim($mobile, '0'); // remove leading zero
$mobile = str_replace(" ","",$mobile);
if (strlen($mobile)!=10){
	$mobile = '7968150172';
}
$check = substr($mobile, 0, 1);
if ($check != 7){
	//invalid mobile - not starting with 7//
	$mobile = '7968150172';
}
//die($mobile);
//$mobile = '7968150172';
			...
			$data = array
						( 	
							'sender_phone' => $mobile, 
							'customer_reference' => $_POST['order_id'], 
							'parcel' => array('size' => 'A'),
							'sender_email'=>'noreply@contact25.com',
							'return_address' => array(
								'building_no' => '56', 
								'city' => 'Leeds', 
								'first_name' => 'Antony', 
							...
							'target_address' => array(
								...
								'building_no' => $building_number[0], 
								'city' => $add_4, 
								'company_name' => ''.ucwords($name).'', 
								'email' => $cust_email, 
								'first_name' => null, 
								'flat_no' => null, 
								'last_name' => null, 
...
#die(var_dump($data));
			...
			$path = '/customers/'.$customerEmail.'/returns'; 
		...
		#die(var_dump($rawResponse));
			...
			$sql = "UPDATE
						order_details
					SET
						order_details.od_tracking_url = '-1',
						order_details.od_tracking_ref = '".$id."'
					WHERE
...
/*
		...
		*/
				...
				//$token = 'fcefb034514b32722173c0480f05d107c6e4cd5ac8e39700f17cf96f2371ba3d'; 
				$parcelNumber = $id; 
				$baseUrl = 'https://'.$location.'.easypack24.net/'; 
				$path = $obj->{_links}->{self}->{href}.'/sticker'; 
				$headers = array( "Authorization: Bearer $token" ); 
				$ch = curl_init() or die (curl_error($ch)); 
				$ch = curl_init($baseUrl.$path);
				...
				//file_put_contents('../../rm_labels/'.$_POST['order_id'].'.pdf', base64_decode($rawResponse)); 
			//	curl_close($ch);
				$pdf = fopen ('../../rm_labels/'.$_POST['order_id'].'.pdf','w');#die("test marker");
				fwrite ($pdf,$rawResponse);
fclose ($pdf);
