...
$sql = 'SELECT 
		...
		FROM 
			spares
		WHERE 
			spares.s_ISBN13 like "%'.$_POST['var_search'].'%"
		AND
...
if ($num_rows >0){
	do{
		...
		<tr>
			<td colspan="2">
				<div style="padding:20px;  font-size:28px; line-height:24px; cursor:pointer; background-color:green;color:white;text-align:center;">(#'.$row['s_num'].') '.$row['s_label'].' (&pound;'.$row['s_price'].') Stock: '.$row['s_qty'].'</div>';
				if ($row['s_qty']>1){
					$result .= '
