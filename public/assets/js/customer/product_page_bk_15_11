    ...
    if(delivery_status=='1') {  // Means single delivery option
      ...
      var sellingprice = parseFloat($('input[name=purchase_val]').val());
        ...
        if (id != '0') {
            ...
            var currencysymbol = $('input[name=currency_symbol]').val();
            $('#paymentheading').text('Pay ' + currencysymbol + totalamount);
...
function lend_option_change(){              // Function to update amount when lending option change
    ...
    $('#paymentheading').text('Pay '+currencysymbol+totalamount);
    $('#purchase_val').val(new_price_update);
    $("#purchase_type").find("option:selected").text("Lend for "+currencysymbol+new_price_update);
...
function collection_type(data){
  ...
  if(show_col_type=='deliver_item')
  {
    ...
    var delivery_status = $('#deliverystatus').val();
    if(deliveryoptioncheck!='0' || delivery_status=='1')   // If user select the delivery option then show payment div
    ...
    else
    {
        $('input[name="delivery_charge"]').val('0');
        var currencysymbol = $('input[name=currency_symbol]').val();
        $('#paymentheading').text('Pay '+currencysymbol+amount);
  ...
  braintree.hostedFields.create({
    ...
    submit.addEventListener('click', function (event) {
      ...
      hostedFieldsInstance.tokenize(function (err, payload) {
        ...
        $.get(mainUrl+'/payment/process', {payload,totalamount,braintree_id,product_id,delivery_provider,formData}, function (response) {
...
function displayPay(){
  ...
  var wallet_amount = parseFloat(document.querySelector('#wallet_amount').value);
	if($('#collect_item').css('display') == 'block'){
      ...
      if(total_amount!='0' && wallet_amount < total_amount)
      {   $('#credit-container').css('display','none');
          $('#card-number').empty();
          $('#expiration-date').empty();
          $('#cvv').empty();
        ...
        //console.log(amount);
         //alert(amount);
         $('#creditbutton').attr('value','Pay ' + currencysymbol + total_amount +' with credit');
        }
      else
      {
        $('#dropin-container').css('display','none');
