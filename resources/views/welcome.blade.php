...
<style>
    ...
    #map {
        width: 100%;height:488px;margin-top:-18px;box-shadow: 5px 6px 12px #ccc;
    }
...
<section class="service-section">
    ...
    <input type="hidden" id="lng" value="<?php echo $location['lon']; ?>">
                            ...
                            <div class="services-area">
                                <div class="services">
                                    @if($p['p_quantity'] < 1)
                                        ...
                                        @if(empty($p['p_image']))
                                            <img src="{{asset('assets/images/logo-balls.png')}}" alt="{{$p['p_title']}} image1">
                                        @else
                                            <?php $p_img_arr = explode(',', $p['p_image']);?>
                                            <img class="d-block w-100" src='{{asset("uploads/products/$p_img_arr[0]")}}' alt="{{$p['p_title']}} image1">
                                        @endif
                                ...
                                @endif
                                 @if(!empty($p['p_item_lend_options']))  
                                    ...
                                    @if($p['p_quantity'] < 1)
                                        <p class="para-text">{{ucwords($p['p_title'])}}
<br>
...
{
              var countryname = '<?php echo $location['country']; ?>';
...
{
    $.ajax({ url:'https://maps.googleapis.com/maps/api/geocode/json?latlng='+lat+','+lng+'&key=AIzaSyD08Emygz5W4HKOZXvogXKb5zYjA8ZRMaQ&sensor=true',
        success: function(data){
